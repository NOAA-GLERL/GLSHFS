...
!--------------------------------------------------------------------
      SUBROUTINE Decode_ExcludeYears(YearString, MetaIndex)
      IMPLICIT NONE
      CHARACTER(LEN=*), INTENT(IN) :: YearString
      INTEGER,          INTENT(IN) :: MetaIndex
      
      INTEGER :: I, J, Y, TYr, SYear, EYear, IOS
      CHARACTER(LEN=5)  :: S1, S2
      CHARACTER(LEN=20) :: ThisStr
      CHARACTER(LEN=50) :: YrStr
      ...
      !
      IF (TYr .GT. 0) THEN
         ALLOCATE(StationMeta(MetaIndex)%ExcludeYears(TYr), STAT=IOS)
         IF (IOS .NE. 0) THEN
            PRINT*, 'Error allocating memory for exclusion years'
            CALL EXIT(1)
         END IF
         DO I = 1, TYr
            StationMeta(MetaIndex)%ExcludeYears(I) = TempYrs(I)
         END DO
      END IF
      RETURN
...
!--------------------------------------------------------------------
      ...
      !    Note on 2021jan28:  
      !    I was having issues with the excluded years stuff. The program was
      !    excluding years that should not have been. Turns out, I needed to
      !    initialize Strings(6), where ParseCommaSepLine returns the excluded 
      !    year string. Since that was a blank section for most stations, the
      !    parse routine wasn't reassigning it. That meant that once a station
      !    had excluded years, those same years would be used for all of the
      !    subsequent stations that had NO excluded years. This is actually a 
      !    bug in the parse routine, so I will now fix it there.
      !
      SNum = 0
      READ(U1, 1000, IOSTAT=IOS) Line
      DO WHILE (IOS .EQ. 0)
         ...
         IF (NumStr .GE. 4) THEN
            ...
            StnID = TRIM(S(1:12))
            S = TRIM(ADJUSTL(Strings(5)))
            StnName = TRIM(S(1:30))
            IF (J1+J2 .EQ. 0) THEN
               SNum = SNum + 1
               StationMeta(SNum)%StnID   = StnID
               StationMeta(SNum)%StnName = StnName
               StationMeta(SNum)%Country = 'xx'
               StationMeta(SNum)%Lat     = SLat
               StationMeta(SNum)%Long    = SLong
               StationMeta(SNum)%UseString = GetUppercase(Strings(4))
               IF (LEN_TRIM(Strings(6)) .GT. 0) THEN
                  CALL Decode_ExcludeYears(Strings(6), SNum)
               END IF
            END IF
         END IF
         READ(U1, 1000, IOSTAT=IOS) Line
...
!--------------------------------------------------------------------
      ...
      DO WHILE (IOS .EQ. 0)
         READ(RestOfLine, 1050, IOSTAT=I) StnName, Country, SLat, SLong
      ...
      DO WHILE (IOS .EQ. 0)
         ...
         IF (I .EQ. 0) THEN
            ...
            StationMeta(SNum)%Lat       = SLat
            StationMeta(SNum)%Long      = SLong
            StationMeta(SNum)%UseString = '...YYYY'    ! by default, use everything for LLTM only
         END IF
         READ(U1, 1000, IOSTAT=IOS) USAF, WBAN, RestOfLine
      END DO
      CLOSE(U1)
 ...
 1050 FORMAT(A30, A2, 12X, F7.3, F9.3) 
...
!--------------  primarily for debugging  ---------
      SUBROUTINE PrintStationInfo
      IMPLICIT NONE
      INTEGER :: I, NumStns
      NumStns = SIZE(StationMeta)
      DO I = 1, NumStns
         WRITE(*, 1000) I, StationMeta(I)%StnID, StationMeta(I)%StnName
      END DO
 1000 FORMAT(I4, ': ', A12, '; ', A30)
      END SUBROUTINE PrintStationInfo

!-----------------------------------------------------------------------------      
      INTEGER FUNCTION GetStationMetaIndex(USAF, WBAN)
      IMPLICIT NONE
      CHARACTER(LEN=6), INTENT(IN) :: USAF
      CHARACTER(LEN=5), INTENT(IN) :: WBAN
      ...
      NumStns = SIZE(StationMeta)
      DO I = 1, NumStns
         IF (StnID .EQ. StationMeta(I)%StnID) THEN
            GetStationMetaIndex = I
            RETURN
         END IF
      END DO
      PRINT*, 'Unable to locate station '//StnID// ' in StationMeta object.'
      PRINT*, 'Processing Aborted.'
      CALL EXIT(1)
      
      END FUNCTION GetStationMetaIndex
...
!-----------------------------------------------------------------------------      
      ...
      INTEGER :: Year, Month, Day, DD, YY, HH, MM, NumDays
      INTEGER :: ErrStat, Seq, SeqStart, SeqFinish
      INTEGER :: HourOffset, NumExcYrs
      LOGICAL :: OK, UseIt
      ...
      INTEGER, DIMENSION(200) :: ExcYrs    ! 200 should be more than enough
      ...
      !
      !  Assign the filtering variables for later use in this routine
      !
      UseString = StationMeta(I)%UseString
      ExcYrs(:) = -9999
      NumExcYrs = 0
      IF (ALLOCATED(StationMeta(I)%ExcludeYears)) THEN
         NumExcYrs = SIZE(StationMeta(I)%ExcludeYears)
         DO J = 1, NumExcYrs
            ExcYrs(J) = StationMeta(I)%ExcludeYears(J)
         END DO
      END IF

      !
      !  Assign station metadata to the big object
      !      
      ...
      MetCsv%DlyData%Lat     = StationMeta(I)%Lat
      MetCsv%DlyData%Long    = StationMeta(I)%Long
      ...
      IF (IOS .NE. 0) THEN
         ...
         CALL EXIT(1)
      END IF
      MetCsv%DlyData%MetData(:,:) = MissingData_Real
      ...
      !
      !  Assign data to the MetCsv%DlyData%MetData arrays
      !  Apply filtering based on the exclusion criteria
      !  that may exist (specified in the master file)
      !
      IF (UseString(2:2) .EQ. 'Y') THEN       ! TMAX is 2nd character of UseString
          DO Seq = SeqStart, SeqFinish
             I = Seq - SeqStart + 1
             UseIt = .TRUE.
             IF (NumExcYrs .GT. 0) THEN
                CALL SequenceDate(DD,MM,YY, Seq)
                DO J = 1, NumExcYrs
                   IF (YY .EQ. ExcYrs(J)) UseIt = .FALSE.
                END DO
             END IF
             IF (UseIt) MetCsv%DlyData%MetData(1,I) = DlyTMax(I)
          END DO
      END IF
      
      IF (UseString(1:1) .EQ. 'Y') THEN        ! TMIN is 1st character of UseString
          DO Seq = SeqStart, SeqFinish
             I = Seq - SeqStart + 1
             UseIt = .TRUE.
             IF (NumExcYrs .GT. 0) THEN
                ...
                END DO
             END IF
             IF (UseIt) MetCsv%DlyData%MetData(2,I) = DlyTMin(I)
          END DO
      END IF
      
      IF (UseString(3:3) .EQ. 'Y') THEN 
          DO Seq = SeqStart, SeqFinish
             I = Seq - SeqStart + 1
             UseIt = .TRUE.
             IF (NumExcYrs .GT. 0) THEN
                ...
                END DO
             END IF
             IF (UseIt) MetCsv%DlyData%MetData(3,I) = DlyPrec(I)
          END DO
      END IF
      
      IF (UseString(4:4) .EQ. 'Y') THEN 
          DO Seq = SeqStart, SeqFinish
             I = Seq - SeqStart + 1
             UseIt = .TRUE.
             IF (NumExcYrs .GT. 0) THEN
                ...
                END DO
             END IF
             IF (UseIt) MetCsv%DlyData%MetData(4,I) = DlyAirT(I)
          END DO
      END IF
      
      IF (UseString(5:5) .EQ. 'Y') THEN 
          DO Seq = SeqStart, SeqFinish
             I = Seq - SeqStart + 1
             UseIt = .TRUE.
             IF (NumExcYrs .GT. 0) THEN
                ...
                END DO
             END IF
             IF (UseIt) MetCsv%DlyData%MetData(5,I) = DlyDewP(I)
          END DO
      END IF
      
      IF (UseString(6:6) .EQ. 'Y') THEN 
          DO Seq = SeqStart, SeqFinish
             I = Seq - SeqStart + 1
             UseIt = .TRUE.
             IF (NumExcYrs .GT. 0) THEN
                ...
                END DO
             END IF
             IF (UseIt) MetCsv%DlyData%MetData(6,I) = DlyWind(I)
          END DO
      END IF
      
      IF (UseString(7:7) .EQ. 'Y') THEN 
          DO Seq = SeqStart, SeqFinish
             I = Seq - SeqStart + 1
             UseIt = .TRUE.
             IF (NumExcYrs .GT. 0) THEN
                ...
                END DO
             END IF
             IF (UseIt) MetCsv%DlyData%MetData(7,I) = DlyClwd(I)
          END DO
      END IF
...
!==============================================================================   
   ...
   IF ((IOS .NE. 0) .OR. ((EndHour .LT. 1) .OR. (EndHour .GT. 24))) THEN
      IF (IOS .EQ. 0) THEN
         ...
         PRINT*, '   Specified EndHour was ', EndHour
      ELSE
         PRINT*, 'Unable to correctly parse EndHour.'
         PRINT*, 'String was [' // Args(3) //']'
      END IF
      PRINT*, 'USAGE: Process_ISH histfile [endhour] filename(s)'
      PRINT*, '     histfile = station metadata file'
      PRINT*, '                Either in the NCEI format (e.g. ish-history.txt) or in'
