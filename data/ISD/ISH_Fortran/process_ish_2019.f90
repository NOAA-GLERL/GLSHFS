...
!--------------------------------------------------------------------
      ...
      DO WHILE (IOS .EQ. 0)
         IF ((TRIM(ID1) .EQ. TRIM(USAF_ID)) .AND.     &
             (TRIM(ID2) .EQ. TRIM(WBAN_ID))) THEN
...
!-----------------------------------------------------------------------------      
      SUBROUTINE ProcessOneStation(HistFileName, DataFileName, EndHour, RetStat)
      IMPLICIT NONE
      ...
      CALL GetStationInfo(HistFileName, USAF_ID, WBAN_ID, StnName,  &
                           Country, StnLat, StnLong)
      
      MetCsv%DlyData%ID      = TRIM(USAF_ID) // '-' // TRIM(WBAN_ID)
      MetCsv%DlyData%Name    = TRIM(StnName)
      MetCsv%DlyData%Country = TRIM(Country)
      MetCsv%DlyData%Lat     = StnLat
      MetCsv%DlyData%Long    = StnLong
...
!==============================================================================   
   ...
   IF (ArgC .LT. 3) THEN
      ...
      PRINT*, 'Note that ish-history.txt must also exist in the current folder.'
      CALL EXIT(1)
   END IF
   ...
   DO I = 4, ArgC
      ...
      CALL ProcessOneStation(HistFileName, DataFileName, EndHour, ErrStat)
